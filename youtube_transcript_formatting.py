...
def process_transcript(transcript, chunk_size=6):
    # Split input text into lines
    lines = transcript.split('\n')
    
    # Lists to store dialogue lines and chunks
    chunks = []

    # Regular expression to identify timestamps in HH:MM:SS format, including all variations (under 1 hour, over 2 hours)
    timestamp_pattern = re.compile(r'^\d{1,2}:\d{2}:\d{2}(?:\.\d{1,3})?$')

    # Initialize variables to store start and end times and manage chunks
    start_time = ""
    end_time = ""
    current_chunk = []
    ...
    for i, line in enumerate(lines):
        ...
        if timestamp_pattern.match(line):
            ...
            if not start_time:
                start_time = line
            # Continuously update the end time to ensure it captures the latest timestamp in the chunk
            end_time = line
            continue
        ...
        # Add dialogue lines to the current chunk
        current_chunk.append(line)

        # Merge lines when reaching the specified chunk size or the end of the transcript
        if len(current_chunk) >= chunk_size or i == len(lines) - 1:
            # Ensure the chunk has valid start and end timestamps
            if start_time and end_time:
                chunks.append((start_time, end_time, ' '.join(current_chunk)))
            # Reset for the next chunk
            current_chunk = []
            start_time = end_time  # Keep the end time as the start of the next chunk
            end_time = ""  # Reset end time for the next iteration

    # Print the formatted chunks with start and end timestamps
    ...
    for i, line in enumerate(lines):
        ...
        if timestamp_pattern.match(line.strip()):
            # Set start time if not set yet, and update end time continuously
            if not start_time:
        ...
        # Append dialogue lines to current chunk
        current_chunk.append(line.strip())

        # Merge every chunk_size lines into a single block
        if len(current_chunk) >= chunk_size or i == len(lines) - 1:
            if current_chunk:
                # Ensure that end_time is set before appending to chunks
                if not end_time:
                    end_time = start_time  # Fallback to start_time if end_time is not set
                chunks.append((start_time, end_time, ' '.join(current_chunk)))
                current_chunk = []
                start_time = end_time
                end_time = ""  # Reset end_time for the next chunk
